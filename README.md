...
- **Link Detection & Handling:** التعرف الذكي على الروابط داخل الرسائل وتسهيل الانتقال إليها.
- **Link Preview:** دعم معاينة الروابط (Link Preview) لعرض تفاصيل الموقع (العنوان، الوصف، الصورة) بشكل مرئي جذاب.
- **Message Details:** إمكانية عرض تفاصيل كل رسالة بشكل مفصل (توقيت الإرسال، الاستلام، والقراءة).
- **Message Selection Mode:** نظام اختيار رسائل متعددة للقيام بعمليات جماعية مثل الحذف أو التوجيه بسلاسة.
- **Local Message Search:** نظام بحث داخلي متطور للبحث عن نصوص محددة داخل المحادثة مع تمييز (Highlighting) النتائج باللون الأصفر والتنقل بينها (Next/Prev) باستخدام `Scrollable.ensureVisible`.
- **Offline Support (Hybrid Loading):** دعم كامل للعمل بدون إنترنت؛ يتم عرض الرسائل وقائمة المحادثات فوراً من قاعدة البيانات المحلية (`sqflite`) ثم تحديثها تلقائياً من `Firestore` باستخدام `MergeStream`.
...
| **rxdart**               | دمج الـ Streams للـ Hybrid Loading  |
| **sqflite**              | التخزين المحلي للرسائل والمحادثات   |
